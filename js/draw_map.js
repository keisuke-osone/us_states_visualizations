...
var TARGET_METRIC = 'Elector_2016'
var WASHINGTON_DC_NAME = 'District of Columbia'
var DRAW_AREA = '#map_area'
var DISPLAY_NAME = 'JPN_expression'
var BUTTON_NAME = 'short_name'

var DATA_NAME = 'presidential_election.csv'
var FILE_NAME = 'us_states.json'
...
var svg_rev_demo = d3.select(DRAW_AREA)
        .append('svg')
        .attr('width', width)
        .attr('height', height);

var legend = d3.select(DRAW_AREA)
        .append('div')
        .attr('id', 'legend')
        .selectAll('div')
...
var demographics = d3.select(DRAW_AREA)
        .append('div')
...
// draw maps
d3.csv(DATA_PATH + DATA_NAME, function(error, data) {
    if (error) throw error;
        election_min = d3.min(data, function (d) {return parseInt(d[TARGET_METRIC]);});
        election_max = d3.max(data, function (d) {return parseInt(d[TARGET_METRIC]);});
    ...
    d3.select('#button_area')
        ...
        .text(function(d) {return d[BUTTON_NAME]});

    d3.json(DATA_PATH + FILE_NAME, function(error, json) {
        if (error) throw error;
        ...
        svg_rev_demo.selectAll('path')
            ...
            .style('fill', function(d) {
                if (d.properties.name == WASHINGTON_DC_NAME) {
                        d3.select(this)
            ...
            .on('mouseover', function(d) {
                metrics = elections[d.properties.name][TARGET_METRIC];
                jpn_name = elections[d.properties.name][DISPLAY_NAME];
            ...
            .on('click', function(d) {
                elector_num = elections[d.properties.name][TARGET_METRIC];
                jpn_name = elections[d.properties.name][DISPLAY_NAME];
                d3.select(this)
                    .transition()
                    .duration(100)
        ...
        demographics.style('opacity', 1.0);
        var elector_num = elections[DEFAULT_STATE_NAME][TARGET_METRIC];
        var jpn_name = elections[DEFAULT_STATE_NAME][DISPLAY_NAME];
        state_name
            .transition()
            .duration(200)
            .text(jpn_name + '州');
    ...
    function getStatesColor (elections, name) {
        for (var i = 0; i < election_threshold.length; i++) {
            if (parseInt(elections[name][TARGET_METRIC]) <= election_threshold[i]) {
                return election_color(election_threshold[i])
            }
        }
    }
